...
def swine_align(player_score, opponent_score):
    ...
    def getGCD(n,m):
        ...
        while n%m:
            n,m=m,n%m
        return m
    GCD=getGCD(player_score,opponent_score)
    if GCD>=10:
        return True
    else:return False
    # END PROBLEM 4a
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    flag=True
    def strategy(who):
        if who==0:return strategy0
        else:return strategy1
    def score(who):
        if who==0:return score0
        else:return score1
    def scoreop(who):
        if who==0:return score1
        else:return score0
    while True:
        num_rolls=strategy(who)(score(who),scoreop(who))
        scorethisturn=score(who)
        scorethisturn+=take_turn(num_rolls,scoreop(who),dice)
        if who==0:score0=scorethisturn
        else:score1=scorethisturn
        say=say(score0,score1)#此处代码有借鉴，之前一直写的是say（score0，score1）没有赋值




#这里有个问题还未得到解答
        #我觉得我大概懂了，返回值确实刷新的函数的参数表，但这个新的函数必须要赋给左值后才会生效，不然就返回到了虚空里
        ...
        if score0>=goal or score1>=goal:
            break
        if not extra_turn(score(who),scoreop(who)):
            who=other(who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    pre_points=0
    while cnt<=10:
        points=(make_averaged(roll_dice,trials_count)(cnt,dice))
