...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    "*** YOUR CODE HERE ***"
    if expressions is nil:return '#t'
    tmp=expressions
    while tmp.rest is not nil:
        tmp2=scheme_eval(tmp.first,env)
        if is_false_primitive(tmp2):return tmp2
        tmp=tmp.rest
    return scheme_eval(tmp.first,env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    if expressions is nil:return '#f'
    tmp=expressions
    while tmp.rest is not nil:
        tmp2=scheme_eval(tmp.first,env)
        if is_true_primitive(tmp2):return tmp2
        tmp=tmp.rest
    return scheme_eval(tmp.first,env)
    "*** YOUR CODE HERE ***"
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        b=bindings.first
        validate_form(b,2,2)
        names,values=Pair(b.first,names),Pair(eval_all(b.rest,env),values)
